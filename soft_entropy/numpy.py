...
def soft_entropy(z: np.ndarray, n_bins: int = 100, seed: int = 0) -> float:
    ...
    w = rng.standard_normal((n_bins, d))
    w = w / np.linalg.norm(w, axis=-1, keepdims=True)  # [n_bins, d]
    ...
    p_per_sample = exp_scores / exp_scores.sum(axis=-1, keepdims=True)  # [batch, n_bins]

    # aggregate over batch to get distribution over bins
    p = p_per_sample.mean(axis=0)  # [n_bins]

    # Shannon entropy
    h = -(p * np.log(np.clip(p, a_min=1e-9, a_max=None))).sum()
